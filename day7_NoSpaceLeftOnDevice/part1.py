...
def main(argv):
    ...
    for item in files:
        if files[item].get_file_type() == 'dir':
            print(f"{files[item].name} is a directory of size {files[item].total()}")
            if files[item].total() <= 100000:
                print(f"Found directory ({files[item].name}) with less than 100000 as size: {files[item].total()}")
